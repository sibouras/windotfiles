/***************************************************************************************************
  ...
  ; from https://autohotkey.com/board/topic/70556-ahk-l-window-dragging-and-resizing-script/
  Features:
  - dragging
  - resizing
  - snapping to screen edges
  - configurable action when mouse touches screen edges (it's hard to explain, must be tried)

  Script is triggered by XButton1. If you want to change it you have to do it in three places in script (I didn't know how to make it possible to set in configuration section)

  How to use?
  Window is divided in 9 cells (width can be defined in configuration section).

  - Resizing:
  Pressing XButton1 in corner of window causes resizing in two dimensions
  Pressing XButton1 between two corners causes resizing in one dimension

  - Dragging:
  Press XButton1 in center of window and move window

  And best functionality (in my opinion)
  If mouse gets to edge window can be snapped to that edge.
  If you move mouse back and then move to close to edge again window can be snapped again but it will have other dimensions (It's configurable, see configuration section).
...
WD_Check:
	if !GetKeyState( "XButton1", "P" )							; Don't check if button is relased
	...
	{ 																; timer checks mouse position.
		...
		{ 															; dragging start but can be disabled
			...
			Loop          ;. Loop checks new mouse position and repositions (and resizes) a window
			{
				...
				if !GetKeyState( "XButton1", "P" )				;. Break if button relased
				{
					WinHide,ahk_id %GuiOutline% ;hide GuiOutline when Mouse is not on any edge
					WinMove,ahk_id %GuiOutline%,,-2000,-2000, ;move Hidden window offScreen,
