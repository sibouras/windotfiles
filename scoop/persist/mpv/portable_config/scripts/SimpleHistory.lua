-- from https://github.com/Eisa01/mpv-scripts
...
function get_list_contents(filter, sort)
  ...
  if filter == "recents" then
    ...
    if
      filePath == list_contents[#list_contents].found_path and tonumber(list_contents[#list_contents].found_time) == 0
    then
    ...
    for i = 1, #list_contents do
      if o.search_behavior == "specific" then
        if string.lower(list_contents[i].found_path):match(string.lower(search_query)) then
          table.insert(filtered_table, list_contents[i])
        elseif
          list_contents[i].found_title and string.lower(list_contents[i].found_title):match(string.lower(search_query))
        then
          table.insert(filtered_table, list_contents[i])
        elseif
          tonumber(list_contents[i].found_time) > 0
          and format_time(list_contents[i].found_time, o.osd_time_format[3], o.osd_time_format[2], o.osd_time_format[1]):match(
            search_query
          )
        then
          table.insert(filtered_table, list_contents[i])
...
function history_resume_notification()
  if not o.resume_notification or not o.osd_messages then
    return
  end
  ...
  if logged_time > 0 then
    ...
    if
      percentage > o.resume_notification_threshold and percentage < (100 - o.resume_notification_threshold)
      or o.resume_notification_threshold == 0
    then
      mp.osd_message(
        "⌨ ["
          ...
          .. "] Resumes To"
          .. o.time_seperator
          .. format_time(logged_time, o.osd_time_format[3], o.osd_time_format[2], o.osd_time_format[1]),
        3
      )
    end
  end
